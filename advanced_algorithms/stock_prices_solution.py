...
The data is in the form of an array with the stock price in 30 minute intervals from 9:30 a.m EST opening to 4:00 p.m EST closing time.
...
>Note: This is a shortened array, just for the sake of example—a full set of prices for the day would have 13 elements (one price for each 30 minute interval betwen 9:30 and 4:00),
as seen in the test cases further down in this notebook.

In order to get the maximum profit in this example, you would want to buy at a price of 3 and sell at a price of 8 to yield a maximum profit of 5.
In other words, you are looking for the greatest possible difference between two numbers in the array.
...
def max_returns(prices):  # [YC] I didn't solve this one, still don't quite understand the solution
    """
...
def max_returns(arr):
    ...
    for index in range(1, len(arr)):
        ...
        # current max profit
        if arr[max_price_index] - arr[min_price_index] < arr[index] - arr[current_min_price_index]:  # [YC] a lot of times (when arr[index] < arr[current_min_price_index]), then index and current_min_price_index are the same???
